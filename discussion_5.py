...
class Warehouse:
	...
	def get_max_stock(self):
		firstStock = self.items[0]
		for item in self.items:
			if firstStock.stock < item.stock:
				firstStock = item
		return firstStock
	
	# Returns the item in the warehouse with the highest price
	def get_max_price(self):
		firstPrice = self.items[0]
		for item in self.items:
			if firstPrice.price < item.price:
				firstPrice = item
		return firstPrice	
...
class TestAllMethods(unittest.TestCase):
	...
	def test_count_a(self):
		self.assertEqual(count_a("Stage"), 1)
		self.assertEqual(count_a("Zero"), 0)
	...
	def test_add_item(self):
		...
		self.assertEqual(len(w1.items), 5)
